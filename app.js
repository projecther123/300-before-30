...
async function createProfile(username) {
  ...
  if (btn) {
    btn.disabled = true;
    btn.textContent = 'Creating your list…';
  }

  try {
    await ensureAnonymousUser();

    const { error } = await db.rpc('create_profile_and_list', {
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = 'Create my list';
    }
  }
}
...
async function loadApp() {
  ...
  (subs || []).forEach(s => {
    if (!subsByGoal.has(s.user_experience_id)) {
...
function goalDone(g) {
  ...
  if (g.type === 'checklist') {
    return !!g.subgoals?.length && g.subgoals.every(s => s.completed);
  }
...
function renderHome() {
  ...
  if ($('#homePct')) $('#homePct').textContent = `${s.pct}%`;
  if ($('#homeCount')) $('#homeCount').innerHTML = `${s.done} of ${s.total}<br>completed`;
...
function cardImageForGoal(g) {
  const position = Number(g.position || 0);
  const filename = String(position).padStart(3, '0');
    ...
    filters.innerHTML = ['All', ...CATS].map(c => `
      ...
      >
        ${
          c === 'All'
            ? 'All'
            : `${ICONS[c]} ${escapeHtml(FILTER_SHORT[c] || c)}`
        }
      </button>
    `).join('');

    $$('#categoryFilters button').forEach(b => {
      b.onclick = () => {
        state.category = b.dataset.cat;
        renderList();
      };
  ...
  wrap.innerHTML = goals.map(g => `
    <article class="goal-card" data-id="${g.id}">
      ...
      <div class="goal-card-copy">
        ...
        <div class="meta">
          ${escapeHtml((CAT_SHORT[g.category] || g.category).toUpperCase())}
        </div>
      </div>
...
function openGoal(id) {
  ...
  let body = `
    <div class="goal-num">${g.position || ''}</div>
    <div class="goal-title">${escapeHtml(g.title)}</div>
    <div class="goal-cat">
      ${ICONS[g.category] || '◇'} ${escapeHtml(g.category)}
    </div>
  `;

  if (g.type === 'standard') {
    body += `
      <button class="complete-btn" id="modalComplete">
        ${goalDone(g) ? '✓ Completed — tap to undo' : '✓ Mark as complete'}
      </button>
    `;
  }
  ...
  if (g.type === 'counter') {
    body += `
      ...
      <div style="text-align:center">of ${g.target}</div>
    `;
  }

  if (g.type === 'checklist') {
    body += (g.subgoals || []).map((s, i) => `
      <label class="subgoal">
        <input
          type="checkbox"
          data-sub="${i}"
  ...
  body += `
    <div class="goal-actions">
      ...
      <button id="deleteFromSheet">Delete</button>
    </div>
  `;

  if ($('#goalContent')) $('#goalContent').innerHTML = body;
  $('#goalModal')?.classList.remove('hidden');

  $('#modalComplete')?.addEventListener('click', () => {
    toggleGoal(id).then(() => openGoal(id));
  });

  $('#minus')?.addEventListener('click', () => {
    setCounter(g, Math.max(0, (+g.current || 0) - 1));
  });

  $('#plus')?.addEventListener('click', () => {
    setCounter(g, Math.min(g.target, (+g.current || 0) + 1));
  });

  $('#counterInput')?.addEventListener('change', e => {
    setCounter(
      g,
      Math.max(0, Math.min(g.target, +e.target.value || 0))
    );
  });

  $$('[data-sub]').forEach(x => {
    x.onchange = () => {
      setSubgoal(g, +x.dataset.sub, x.checked);
    };
  });

  if ($('#editGoal')) $('#editGoal').onclick = () => openEdit(g);
  if ($('#deleteFromSheet')) $('#deleteFromSheet').onclick = () => deleteGoal(g);
}
...
async function setSubgoal(g, index, checked) {
  ...
  s.completed = checked;
  g.completed = g.subgoals.every(x => x.completed);
...
function renderMe() {
  const s = stats();
  const name = currentUsername || 'profile';

  if ($('#profileName')) $('#profileName').textContent = '@' + name;
  if ($('#avatar')) $('#avatar').textContent = name.slice(0, 2).toUpperCase();
  if ($('#mePct')) $('#mePct').textContent = `${s.pct}%`;
  if ($('#meDone')) $('#meDone').textContent = `${s.done} completed`;
  if ($('#meRemaining')) $('#meRemaining').textContent = `${s.remaining} remaining`;
  if ($('#meTotal')) $('#meTotal').textContent = `${s.total} total`;
}
...
function openEdit(g = null) {
  ...
  if ($('#editId')) $('#editId').value = g?.id || '';
  if ($('#editTitle')) $('#editTitle').value = g?.title || '';
  if ($('#editCategory')) $('#editCategory').value = g?.category || CATS[0];
...
async function saveEditedGoal() {
  ...
  const category = $('#editCategory')?.value || CATS[0];
  ...
  const nextPosition =
    Math.max(0, ...state.goals.map(g => +g.position || 0)) + 1;
  ...
  if (error) {
    showError('Couldn’t add that experience.');
    return;
  }
...
async function resetToMaster() {
  ...
  try {
    ...
    const { data: newGoals, error: iErr } = await db
      .from('user_experiences')
      .insert(payload)
      .select('id,master_experience_id');

    if (iErr) throw iErr;

    const idByMaster = new Map(
      newGoals.map(g => [g.master_experience_id, g.id])
    );

    const subPayload = (masterSubs || [])
      .map(s => ({
        user_experience_id: idByMaster.get(s.master_experience_id),
        user_id: currentUser.id,
        position: s.position,
        title: s.title,
        completed: false
      }))
      .filter(x => x.user_experience_id);
    ...
    console.error(err);
    hideLoading();
    showError('Reset failed. Your current list may need reloading.');
    await loadApp();
  }
}
...
async function deleteProfile() {
  ...
  try {
    ...
    console.error(err);
    hideLoading();
    showError('Couldn’t delete the profile.');
  }
}
...
function exportList() {
  ...
  a.download = `300-before-30-${currentUsername}.json`;
  a.click();
...
function bind() {
  ...
  if ($('#addGoal')) $('#addGoal').onclick = () => openEdit();
  ...
  if ($('#switchProfile')) $('#switchProfile').onclick = startAnotherProfile;
  if ($('#resetBtn')) $('#resetBtn').onclick = resetToMaster;
  if ($('#deleteProfile')) $('#deleteProfile').onclick = deleteProfile;
}
